...
const Analysis = () => {
    ...
    const getFieldAdvice = (data) => {
        ...
        const { temperature, soilMoisture, waterLevel, soundFrequency, animalPresence } = data;
        let advice = [];
        ...
        if (temperature > 30) {
            advice.push({
                icon: '🌡️',
                title: 'Temperature',
                message: 'It\'s very hot. Consider providing shade and additional watering.',
                bgColor: 'bg-red-200'
            });
    ...
    if (loading) return <div className='flex justify-center items-center'><span className="loading loading-spinner text-success"></span></div>;
        ...
        <div className="">
            <div className="flex flex-col mb-10 md:flex-row bg-gray-100 rounded-2xl">
                ...
                <div className="flex-1 p-5 md:p-10">
                    ...
                    {floodOrDroughtRisk && (
                        <div className={`mb-8 p-5 rounded-lg shadow-lg ${floodOrDroughtRisk.includes('High') ? 'bg-red-200 text-red-800' : 'bg-yellow-200 text-yellow-800'}`}>
                            <h2 className="text-2xl font-semibold mb-2">⚠️ Risk Assessment</h2>
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-lg">
                        <h2 className="text-3xl font-bold mb-5">🌾 Field Recommendations</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {sensorData ? (
                                getFieldAdvice(sensorData).map((item, index) => (
                                    <div key={index} className={`${item.bgColor} p-5 rounded-lg shadow-md flex flex-col items-center`}>
                    ...
                    {insights && (
                        <div className="mt-10">
                            <h2 className="text-3xl font-bold mb-5">📈 Predictions for Next Day</h2>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                <div className="bg-white p-6 rounded-lg shadow-lg">
                                    <h2 className="text-2xl font-semibold mb-4">Humidity Trend</h2>
                                    <p className={`text-3xl ${insights.humidityTrend > 0 ? 'text-red-600' : 'text-green-600'}`}>
                                        {insights.humidityTrend.toFixed(2)}
                                    </p>
                                </div>

                                <div className="bg-white p-6 rounded-lg shadow-lg">
                                    <h2 className="text-2xl font-semibold mb-4">Precipitation Trend</h2>
                                    <p className={`text-3xl ${insights.precipitationTrend > 0 ? 'text-red-600' : 'text-green-600'}`}>
                                        {insights.precipitationTrend.toFixed(2)}
                                    </p>
                                </div>

                                <div className="bg-white p-6 rounded-lg shadow-lg">
                                    <h2 className="text-2xl font-semibold mb-4">Soil Moisture Trend</h2>
                                    <p className={`text-3xl ${insights.soilMoistureTrend > 0 ? 'text-red-600' : 'text-green-600'}`}>
                                ...
                                <div className="bg-white p-6 rounded-lg shadow-lg">
                                    ...
                                    <p className={`text-3xl ${insights.temperatureTrend > 0 ? 'text-red-600' : 'text-green-600'}`}>
                                ...
                                <div className="bg-white p-6 rounded-lg shadow-lg">
                                    ...
                                    <p className={`text-3xl ${insights.windTrend > 0 ? 'text-red-600' : 'text-green-600'}`}>
